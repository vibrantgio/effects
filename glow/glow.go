...
//
// A blur-based glow gives that true radial falloff with no tile seams,
// and is deliberately not offered: it costs 0.2–0.8 ms of
// events-thread CPU plus an image-sized allocation and a texture
// upload per glow per frame, against the gradient path's ~0.5 µs of op
// recording (ten-core Apple Silicon, darwin/arm64, go test -bench).
// Caching cannot recover that, because the glows worth having animate
// Radius or Intensity and so miss any shape-keyed cache every frame. A
// prototype, the falloff comparison and the benchmarks live in
